...
/**
 * metadata優先で organization_id を取得
 * 旧prefixは fallback
 */
function getOrganizationId(event: calendar_v3.Schema$Event): string | null {
  const sharedId = String(
    event.extendedProperties?.shared?.organization_id || ''
  ).trim();

  if (/^\d{6}$/.test(sharedId)) {
    return sharedId;
  }

  const title = getEventTitle(event);
  return getMeetingId(title);
}

/**
 * タイトル先頭の旧prefixを除去
 */
function getTitleWithoutIdPrefix(title: string): string {
  return title.replace(/^\d{6}_/, '').trim();
}

/**
 * フォルダ名を「ID_案件名」の形式で取得する
 */
function getFolderNameFromEvent(event: calendar_v3.Schema$Event): string {
  const title = getEventTitle(event);
  const organizationId = getOrganizationId(event);
  const cleanTitle = getTitleWithoutIdPrefix(title);

  if (organizationId) {
    const rawName = cleanTitle.split(/[\s_]/)[0];
    return `${organizationId}_${sanitize(rawName)}`;
  }

  return sanitize(cleanTitle);
}
...
function findExistingMeetingFolder(baseDir: string, meetingId: string): string | null {
  ...
  try {
    ...
    const found = entries.find(
      (entry) => entry.isDirectory() && entry.name.startsWith(`${meetingId}_`)
    );
...
/**
 * 指定ディレクトリ直下の最新更新ファイルのパスを返す
 * @param dir
 * @return
 */
function getLatestFilePath(dir: string): string | null {
...
/**
 * カレンダーの添付Googleドキュメントを取得し、メモ（なければ全文）を保存する
 * @param event 
 * @param dir 
 * @param targetDate 
 */
  ...
  for (const att of attachments) {
    if (att.mimeType === 'application/vnd.google-apps.document') {
      try {
        ...
        const message =
          fileErr?.response?.data?.error?.message ??
          fileErr?.message ??
          'unknown error';
...
function getMemoUrl(event: calendar_v3.Schema$Event): string | null {
  if (!event.attachments) return null;
	// Googleドキュメントを優先して探す
  const doc = event.attachments.find(
...
function buildTomorrowTask(
  ...
  const prevTitle = yesterdayEvent ? getEventTitle(yesterdayEvent) : '';
  const prevMemoUrl = yesterdayEvent ? getMemoUrl(yesterdayEvent) : null;
...
async function runActionA() {
  ...
  const targetEmails = Array.from(
    new Set(
      sharedItems.data.files
        ?.flatMap((f) => f.owners?.map((o) => o.emailAddress))
  ...
  if (targetAccount && emailsToProcess.length === 0) {
    console.log(
      `⚠️ TARGET_ACCOUNT に指定されたユーザーが共有一覧に見つかりません: ${targetAccount}`
    );
    return;
  }
  ...
  for (const email of emailsToProcess) {
    try {
      ...
      const yesterdayEventByOrganizationId = new Map<string, calendar_v3.Schema$Event>();
      ...
      for (const event of events) {
        ...
        const organizationId = getOrganizationId(event);
				...
				// mataDataに組織IDが存在しない場合は除外
				if (!organizationId) {
          console.log(`⏩ スキップ: ${title} (organization_idなし)`);
          continue;
        }

        let folderName = getFolderNameFromEvent(event);

        const existingPath = findExistingMeetingFolder('meetings', organizationId);
        if (existingPath) {
        ...
        if (!yesterdayEventByOrganizationId.has(organizationId)) {
          yesterdayEventByOrganizationId.set(organizationId, event);
        }
      }

      for (const tomorrowEvent of tomorrowEvents) {
        const tomorrowTitle = getEventTitle(tomorrowEvent);
        const organizationId = getOrganizationId(tomorrowEvent);

        if (!organizationId) {
          console.log(
            `⏩ 明日予定をスキップ: ${tomorrowTitle} (organization_idなし)`
          );
          continue;
        }
        ...
        if (!existingPath) {
          console.log(
            `⏩ 明日予定をスキップ: ${tomorrowTitle} (meetings配下に同一IDフォルダなし)`
          );
          continue;
        }

        const folderName = path.basename(existingPath);
        const latestFilePath = getLatestFilePath(existingPath);
        const yesterdayEvent = yesterdayEventByOrganizationId.get(organizationId);
        ...
        writeTomorrowTaskFile('inbox', folderName, tomorrow, tomorrowTask);
      }
    } catch (err: any) {
